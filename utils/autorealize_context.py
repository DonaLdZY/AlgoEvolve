"""AutoRealize context adapter for AlgoEvolve.

AlgoEvolve can still run standalone with only a data folder and description.md.
...
def select_autorealize_context_for_stage(text: str, stage: str) -> str:
    ...
    inputs are returned unchanged so standalone AlgoEvolve behavior is preserved.
    """
...
def _pack_is_prompt_ready(pack: Any) -> bool:
    ...
    audit material, but they are not enough to replace AlgoEvolve's standalone data
    preview. Accept only packs that carry the downstream contracts AlgoEvolve needs.
    """
...
def _render_from_pack(pack: dict[str, Any]) -> str:
    lines = [
        ...
        "When this context is present, AlgoEvolve must not regenerate or append a separate data preview; AutoRealize has already authored the stable task/data context.",
        "",
    ]
...
def _render_fallback(contracts: dict[str, Any]) -> str:
    ...
    lines = [
        f"## {CONTEXT_MARKER}",
        "",
        "AutoRealize structured artifacts were detected. Treat them as high-priority task facts and do not regenerate a raw AlgoEvolve data preview.",
        "",
        "### Problem And Goal",
...
def build_autorealize_context_md(input_dir: Path, *, write_context_file: bool = True) -> str:
    ...
    AlgoEvolve must not recreate data cognition when an AutoRealize package is
    available. AutoRealize owns context construction; this adapter only copies
    the already-authored prompt into the AlgoEvolve input directory.
    """
    contracts = _load_contracts(Path(input_dir))
    ...
    if not text:
        ...
        if _markdown_is_prompt_ready(fallback):
            ...
            logger.warning(
                "AutoRealize artifacts exist but are not prompt-ready for AlgoEvolve; "
...
def load_autorealize_description_md(input_dir: Path, *, fallback: str = "") -> str:
    """Return reader-facing description.md for AlgoEvolve task_desc."""
    contracts = _load_contracts(Path(input_dir))
...
def submission_required_from_context(input_dir: Path) -> bool | None:
    """Infer whether AlgoEvolve should require ./submission/submission.csv."""
    contracts = _load_contracts(Path(input_dir))
